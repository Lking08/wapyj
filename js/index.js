$(function(){
	...
	$(".cy-pose p b").click(function(){
		var i=$(this).closest(".cy-pose").index();
	...
	function costChange(){
		...
		$.each($(".shop-cart-list a"),function(i,n){
			...
			if($(this).hasClass("active")){
				...
				arr2.push(age);
				//更新购物车总价格
				var ztotal=eval(arr1.join('+'));
				var ntotal=eval(arr2.join('+'));
				$(".shop-check-all").find("#sum").html(ztotal);
				//更新购物车结算的个数
				$(".shop-check-all").find("#number").html(ntotal);
			}else{
				$(".shop-check-all").find("#sum").html( 0 + '.' + 0);
	...
	$(".shop-cart-list dd span").find("em").click(function(){
		var i=$(this).closest(".shop-cart-list").index();
		var num=$(".shop-cart-list dd span").eq(i).find("input").val();
		num-- ;
		var minNum = parseInt(num--);
		if(minNum < 0){
			minNum = 0;
		}
		$(".shop-cart-list dd span").eq(i).find("input").val(minNum);
	});
	//购物车单个物品购买个数加一
